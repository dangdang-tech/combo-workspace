---
title: OIDC auth
description: Configure Google and generic OIDC providers (Okta/Auth0/Azure AD/Keycloak, etc.) for a self-hosted server.
---
...
- `allow` restricts eligibility (all string comparisons are lowercased):
  - `requireVerifiedEmail` (default `false`): requires an email claim and the standard `email_verified` claim to be the boolean `true`. Missing, false, or string-valued verification claims are rejected on linking and login.
...
## Google: keyed accounts with E2EE

Create a Google OAuth **Web application** client and register the exact server callback URI, for example `https://YOUR_SERVER/v1/oauth/google/callback`. Configure the consent screen and any Google test users before attempting a live login. Use the existing OIDC provider with id `google`:
...
[
  {
    ...
    "redirectUrl": "https://YOUR_SERVER/v1/oauth/google/callback",
    "scopes": "openid profile email",
...
Store this JSON in the server's private configuration file, then configure:
...
HAPPIER_WEBAPP_URL=https://YOUR_WEB_APP
...
This uses keyed signup: the client creates its own keys and proves possession during finalization. For encrypted sharing recipients, verify that `GET /v1/features` exposes `sharing.session.enabled: true`; the client then registers its signed content public key during signup. Keep the provider id exactly `google` for Google-specific sharing eligibility.

Google's `sub` is the stable account identity; an email change does not create a different identity. Google does not supply `preferred_username`, so Happier uses the verified email as the provider login and asks a new user to choose a Happier username. The configured display name appears on the sign-in option; icon hints do not guarantee a Google-branded button.

`emailDomains` checks only the email suffix. It is not a Google Workspace membership check: Google's `hd` claim represents hosted-domain membership, and this configuration does not enforce that claim. See [Google's OpenID Connect documentation](https://developers.google.com/identity/openid-connect/openid-connect) for client setup and claim semantics.

Local signed OIDC integration tests validate Google-shaped claims, rejection of unverified email, and keyed E2EE finalization. A real Google consent/redirect flow still requires your registered client, secret, deployed URLs, and a live browser login.
